...
internals.generatePolicy = function (options, request) {
  ...
  internals.directiveNames.forEach((key) => {
    ...
    if ((key === 'scriptSrc' || key === 'styleSrc') && internals.nonceShouldBeGenerated(options, key)) {
      const nonce = request.plugins.contentSecurityPolicy ? request.plugins.contentSecurityPolicy.nonces[internals.keyToConfigMap[key]] : internals.generateNonce()
...
internals.attachNonces = function (request, h) {
  ...
  if (request.route.settings.plugins.contentSecurityPolicy) {
    options = internals.validateOptions(request.route.settings.plugins.contentSecurityPolicy)
  ...
  ['scriptSrc', 'styleSrc'].forEach((key) => {
    if (internals.nonceShouldBeGenerated(options, key)) {
      ...
      request.plugins.contentSecurityPolicy.nonces[internals.keyToConfigMap[key]] = nonce
    }
  })
